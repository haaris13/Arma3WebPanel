...
$resultQ = 'SELECT uid FROM listallplayers';
...
if (isset($_POST['search'])) {
    ...
    $sqlget = "SELECT * FROM listallplayers WHERE CONCAT (`name`,`playerid`,`uid`, `aliases`) LIKE '%".$valuetosearch."%'";
    $search_result = filterTable($dbcon, $sqlget);
    if ($search_result == '') {
        $sqlget = "SELECT * FROM listallplayers WHERE CONCAT (`name`,`pid`,`uid`, `aliases`) LIKE '%".$valuetosearch."%'";
        $search_result = filterTable($dbcon, $sqlget);
    }
} elseif (isset($_POST['orderBank']) || $_GET['search'] == 'bank') {
    $sqlget = 'SELECT * FROM listallplayers ORDER BY bankacc DESC limit '.$page.',50';
    $search_result = filterTable($dbcon, $sqlget);
} elseif (isset($_POST['orderCash']) || $_GET['search'] == 'cash') {
    $sqlget = 'SELECT * FROM listallplayers ORDER BY cash DESC limit '.$page.',50';
    $search_result = filterTable($dbcon, $sqlget);
} elseif (isset($_POST['orderCop']) || $_GET['search'] == 'cop') {
    $sqlget = 'SELECT * FROM listallplayers ORDER BY coplevel DESC limit '.$page.',50';
    $search_result = filterTable($dbcon, $sqlget);
} elseif (isset($_POST['orderMedic']) || $_GET['search'] == 'medic') {
    $sqlget = 'SELECT * FROM listallplayers ORDER BY mediclevel DESC limit '.$page.',50';
    $search_result = filterTable($dbcon, $sqlget);
} elseif (isset($_POST['orderAdmin']) || $_GET['search'] == 'admin') {
    $sqlget = 'SELECT * FROM listallplayers ORDER BY adminlevel DESC limit '.$page.',50';
    $search_result = filterTable($dbcon, $sqlget);
} else {
    $sqlget = 'SELECT * FROM listallplayers limit '.$page.',50';
    $search_result = filterTable($dbcon, $sqlget);
}
include 'header/header.php';
?>
<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
    <h1 style = "margin-top: 70px">Menu du joueur</h1>
    <p class="page-header">Ce menu vous permet de changer/voir certaine information sur le joueur </p>
    ...
    switch ($staffPerms) {
        ...
        case $staffPerms['notes'] == '1':
            echo "<div class=\"btn-group\" role=\"group\" aria-label=\"...\">
			              ...
			              <INPUT class='btn btn-primary btn-outline' TYPE=\"submit\" VALUE=\"Ajouter une note\">
			              </FORM>
		                  </div>";
    }
    ?>
    ...
    <div class="table-responsive">
        <table class="table table-striped" style = "margin-top: 0px">
            ...
            while ($row = mysqli_fetch_array($search_result, MYSQLI_ASSOC)) {
                ...
                echo '<td>'."<input class='btn btn-primary btn-outline' type=submit name=edit id=edit value=Voir".' ></td>';
